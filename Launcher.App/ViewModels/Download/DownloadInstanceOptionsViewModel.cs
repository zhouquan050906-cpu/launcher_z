...
public sealed partial class DownloadInstanceOptionsViewModel : ObservableObject, IDisposable
{
    ...
    public bool CanInstall => isActive
        ...
        && !IsCheckingInstanceNameAvailability
    ...
    public async Task RefreshNameAvailabilityAsync(CancellationToken cancellationToken = default)
    {
        if (!isActive)
            return;
    ...
    partial void OnIsCheckingInstanceNameAvailabilityChanged(bool value)
    {
        NotifyStateChanged();
    }
    ...
    private async Task RefreshExistingInstanceNamesAsync(CancellationToken cancellationToken)
    {
        ...
        await CheckInstanceNameAvailabilityAsync(
            InstanceName,
            debounce: false,
            cancellationToken);
    }
    ...
    private void RefreshInstanceNameDuplicateMessage()
    {
        ...
        InstanceNameDuplicateMessage = !string.IsNullOrWhiteSpace(normalizedName)
            && (instanceNameTracker.IsUnavailable(normalizedName)
                || string.Equals(
                    externallyUnavailableInstanceName,
                    normalizedName,
    ...
    private void RequestInstanceNameAvailabilityCheck()
    {
        ...
        _ = CheckInstanceNameAvailabilityAsync(
            normalizedName,
            debounce: true,
            CancellationToken.None);
    }

    private async Task CheckInstanceNameAvailabilityAsync(
        string? instanceName,
        bool debounce,
        CancellationToken cancellationToken)
    {
        ...
        var previousCancellation = Interlocked.Exchange(
            ref nameAvailabilityCancellation,
            requestCancellation);
        ...
        try
        {
            ...
            externallyUnavailableInstanceName = availability is InstanceInstallNameAvailability.Occupied
                ? normalizedName
                : string.Empty;
            RefreshInstanceNameDuplicateMessage();
        }
        ...
        catch (Exception exception)
        {
            ...
            logger.LogWarning(
                exception,
                "Failed to refresh Minecraft instance name availability. InstanceName={InstanceName}",
                normalizedName);
        }
        finally
        {
    ...
    private bool IsNameAvailabilityRequestCurrent(
        CancellationTokenSource requestCancellation,
        string instanceName) =>
        ReferenceEquals(nameAvailabilityCancellation, requestCancellation)
        && isActive
        && string.Equals(InstanceName?.Trim(), instanceName, StringComparison.OrdinalIgnoreCase);

    private void CancelNameAvailabilityRequest()
    {
        var cancellation = Interlocked.Exchange(ref nameAvailabilityCancellation, null);
        cancellation?.Cancel();
        IsCheckingInstanceNameAvailability = false;
    }
